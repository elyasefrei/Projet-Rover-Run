...
Notre projet consiste à créer un mini-jeu d'un robot sur Mars qui doit retrouver son chemin pour aller à la base. Chaque phase le robot devra trouver la combinaison de mouvement la plus efficace pour être le plus proche de la base qui est la condition de fin de notre jeu.
...
2) Afficher une map où on a les costs , et une map où on a les différents types de terrain possibles.
...
1) On va d'abord créer la racine de l'arbre avec un create_node
2) Ensuite avec cette racine on va pouvoir créer tous les noeuds d'arbres avec create_children(root,max_depth).
...
4) Par la suite on a créé le tableau aléatoire qui va être découper en deux partie:
...
5) Après la création du tableau avec les mouvements aléatoires on peut enfin remplir notre arbre avec notre fonction "remplir_arbre"
...
### 4) Trouver le chemin avec le coût minimal 
